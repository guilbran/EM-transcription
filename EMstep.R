...
Vsmooth <- as.matrix(read.csv("arquivos pra fç EMstep/Vsmooth.csv", header = F))
VVsmooth <- as.matrix(read.csv("arquivos pra fç EMstep/VVsmooth.csv", header = F))
loglik <- as.matrix(read.csv("arquivos pra fç EMstep/loglik.csv", header = F))

EMstep <- EMstep(y = NULL, A = NULL, C = NULL, Q = NULL, R = NULL, Z_0 = NULL, V_0 = NULL, 
...
for(i in 1:n_b){
  ...
  EZZ <- Zsmooth[(rp1+1):(rp1+rp),2:ncol(Zsmooth)] %*% t(Zsmooth[(rp1+1):(rp1+rp),2:ncol(Zsmooth)]) + colSums(Vsmooth[(rp1+1):(rp1+rp),(rp1+1):(rp1+rp),2:end])[3]  # E(Z'Z)
  EZZ_BB <- Zsmooth[(rp1+1):(rp1+rp),1:(ncol(Zsmooth)-1)] %*% t(Zsmooth[(rp1+1):(rp1+rp),1:(ncol(Zsmooth)-1)]) + colSums(Vsmooth[(rp1+1):(rp1+rp),(rp1+1):(rp1+rp),1:(ncol(Zsmooth)-1)])[3] #E(Z(-1)'Z_(-1))
  EZZ_FB <- Zsmooth[(rp1+1):(rp1+rp),2:ncol(Zsmooth)] %*% t(Zsmooth[(rp1+1):(rp1+rp),1:(ncol(Zsmooth)-1)]) + colSums(VVsmooth[(rp1+1):(rp1+rp),(rp1+1):(rp1+rp),])[3] #E(Z'Z_(-1))
